...
> [**Markdown 文档查看帮助**](Markdown文档查看帮助.md)（也附了 PDF：`Markdown文档查看帮助.pdf`）
>
> 🎨 **用 VSCode 打开整个文件夹**，预览会自动套上本仓库自带的配色与高亮方案
> （提示词块会高亮成琥珀色卡片），无需任何设置。详见[查看帮助 · 方式 1](Markdown文档查看帮助.md#方式-1--vscode--预览推荐免费)
>
...
| # | 文档 | 你会得到什么 | 建议用时 |
| :---: | :--- | :--- | :---: |
| **1** | [装好 VSCode 与 WorkBuddy，并汉化成中文](AI编程环境搭建/01-VSCode与WorkBuddy安装与汉化.md) | VSCode 装好、界面变中文、WorkBuddy 能登录、**免费积分全部领到手** | 15 min |
| **2** | [认识 ZCode，搞清工具与模型怎么选](AI编程环境搭建/02-ZCode与WorkBuddy对比及模型选型.md) | WorkBuddy vs ZCode、GLM 套餐 vs DeepSeek API；**接 DeepSeek API 为可选进阶（需充值，可跳过）** | 15 min |
| **3** | [用 WorkBuddy 装好 uv 和 Python 3.12](AI编程环境搭建/03-用WorkBuddy安装uv与Python3.12.md) | 搞清 **uv 是什么、强在哪**，再用一份**可复制的提示词**让 AI 装好 uv + Python（**只装系统环境**） | 15 min |
| **4** | [在 VSCode 里配环境，跑通 Hello World](AI编程环境搭建/04-VSCode终端配置环境与HelloWorld.md) | 打开文件夹 → 开终端 → uv 建环境 → 写 `hello_world.py` → **点 ▶ 运行**（手动路线） | 20 min |
| **5** | [用 WorkBuddy 配好工作区并运行代码](AI编程环境搭建/05-用WorkBuddy配置工作区并运行代码.md) | 设工作区 → 一句话让 AI 建环境写代码 → VSCode 打开 → 点运行（**AI 路线**） | 15 min |
...
| # | 文档 | 你会得到什么 | 提示词 |
| :---: | :--- | :--- | :---: |
| — | [**传感器监控平台 · 导读**](传感器监控平台/README.md) | 硬件清单、**485 集线器拓扑**、接口文档清单、**协议摘要参考答案（验收用）** | — |
| **1** | [环境搭建与项目骨架](传感器监控平台/01-环境搭建与项目骨架.md) | uv + Python 3.12 + pymodbus / FastAPI 全套依赖 + 规范目录 | 1 段 |
| **2** | [串口链路打通与本地模拟器](传感器监控平台/02-串口链路打通与模拟器.md) | **让 AI 读接口文档产出协议摘要** + 串口工具 + 免硬件的本地模拟器 | 2 段 |
| **3** | [真机接入与总线组网](传感器监控平台/03-真机接入与总线组网.md) | **RS485 总线原理 + 逐台改从机地址 + 统一波特率 + 485 集线器组网与总线联调** | 3 段 |
| **4** | [三个传感器驱动开发](传感器监控平台/04-三个传感器驱动开发.md) | 统一基类 + 三个驱动类 + 三个调试脚本（**核心**） | 2 段 |
| **5** | [实时采集与控制后端](传感器监控平台/05-实时采集与控制后端.md) | FastAPI 轮询采集 + 缓存 + WebSocket 推送 + 控制接口 | 1 段 |
| **6** | [前端网页与联调测试](传感器监控平台/06-前端网页与联调测试.md) | 单文件网页 + pytest 自动化测试 + 真机整机验收清单 | 2 段 |
...
[第 1 篇](传感器监控平台/01-环境搭建与项目骨架.md) 是前置环境；
[第 3 篇](传感器监控平台/03-真机接入与总线组网.md) 是**真机环节的分水岭**——
不接硬件时用它了解总线机制，接了硬件就必须先过它这一关；
[第 6 篇](传感器监控平台/06-前端网页与联调测试.md) 末尾有整机验收清单。
...
> 具体做法见 [该套导读](传感器监控平台/README.md)；答案只在导读末尾的**附录 A**里。
...
**开始阅读** → [第 1 篇 · 装好 VSCode 与 WorkBuddy](AI编程环境搭建/01-VSCode与WorkBuddy安装与汉化.md)
　|　**想直接做项目** → [传感器监控平台](传感器监控平台/README.md)
　|　**不知道怎么打开 md** → [Markdown 文档查看帮助](Markdown文档查看帮助.md)
